...
void stringCallback(char *myString)
{
    ...
    String command = String(myString);
    if ( command == "AC:toggle" ) {
        emitIRSignal();
        Firmata.sendString("AC:toggled");
        return;
    } else if ( command == "TH:get") {
        h = (int) dht.readHumidity();
        t = (int) dht.readTemperature();
        if (isnan(h) || isnan(t)) {
            Firmata.sendString("TH:er,er");
            return;
        }

        String responseStr = String("TH:" + String(t) + "," + String(h));
        char response[responseStr.length() + 1];
